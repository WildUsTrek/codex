...
- V302 current visual contract is `PERLA1_V302_STEADY_QUALITY_FORCE30_LOCAL`: V302 is a steady-quality visual/performance layer over V301. It stabilizes floor/road quantization and far floor banding, makes long-view sprite/floor budgeting scene-driven instead of previous-frame-driven in steady mode, makes protected-large sprite stripe steps deterministic, suppresses far cover/canopy emergency oscillation, suppresses V300-driven roof primitive step changes, and adds a reversible force-30 FPS frame gate. It is not a new roof authority, not a map/collision mutation, not an asset mutation, not a rain output change, not a worldgen change, and not permission to raw-delete public diagnostics.
- V299 remains the clear/non-storm full-surface roof fill step budget for `face.kind === 'roof'`; V298 remains the storm/rain pressure escalation layer. V300/V301 may raise protected quality-budget pressure for sprite/floor and coverage from measured draw/frame/candidate/stripe/floor/coverage/transition signals, while V302 prevents those pressure signals from causing visible steady-state roof/floor/canopy alternation. Edge lines, gable outlines, portal underside, public fallback behavior, V281 primitive preflight, map/assets/rain output, and visual stability remain protected.
- V296 remains cleanup-only history: V293/V294/V295 compatibility remains callable, V294 keeps the V237 safe-emerging-wall implementation tombstoned, V295 keeps the preferred read-only `window.perlaDebugV295` hub, and V296 physically prunes only the proved-dead implementation bodies for V239-V241C high-wall diagnostics and failed roof/eave branches V266/V267/V270/V271/V272/V273/V274/V275. Public summary/toggle/download/status symbols remain compatibility shims and `safeDeleteNow` remains empty for public APIs.
- V292 remains the active wall/replay and column occlusion functional contract, while the V281 modern owner `1`/`2` world-space primitive authority and the V283 canopy foreground guard remain the active visual/roof/canopy contracts. `PERLA_V292_LOCAL_WALL_REPLAY_REQUIRES_ACTIVE_RAIN_REPLAY_SAFE` prevents V220 local wall replay after V216 unless rain/V216 replay context is active, and `PERLA_V292_GAME_ROOM_COLUMN_DIRECT_WALL_OCCLUSION_SAFE` lets the already-drawn direct owner-1 reception wall occlude the sala giochi/game-room canopy column. V291 tamarisk foreground restore remains inactive diagnostics in this build.
- V281S keeps the owner `1` `back_y0` gable only in the real posterior/far-closure or reception portal band so the colmo cannot disappear under same-coordinate rotation. V281T adds an owner `2` door-derived `back_x0` portal gable keep and opposite `front_x1` closure keep for the bath roof; it must use `roof.doors`, not hardcoded coordinates. V281U promotes owner2 to the same full-surface primitive authority class as reception; V281V suppresses owner2 full-surface exterior decorative edge/ridge/eave lines; V281W suppresses owner2 full-surface exterior fascia/gronda faces so the bath roof no longer shows a thick dark strip between wall and roof. V281X applies the same flush wall join used by reception to owner2 and adds a V281-local owner2 foreground-top rejection so bath roof/eave pixels do not draw over unrelated foreground hedge/wall tops.
...
- Expected V283/V281 debug pattern in affected roof and canopy views:
  - `window.PERLA_BUILD_ID` is `PERLA1_V302_STEADY_QUALITY_FORCE30_LOCAL`.
  ...
  - `modernStableRoofPrimitiveV299` is `true`; `modernStableRoofPrimitiveGeneralBudgetAppliedV299` and `modernStableRoofPrimitiveGeneralBudgetColumnsSavedEstimateV299` remain expected in owner `1`/`2` full-surface roof views in clear/non-storm weather.
  - `smartAdaptiveV300` is `true`; when `force30FpsV302` is off, `smartAdaptiveLevelV300`, `smartAdaptiveTargetFpsV300`, `longViewAdaptiveLevelV300`, and `coverageAdaptiveLevelV300` are V300 proof counters for measured pressure. When `force30FpsV302` is on, `smartAdaptiveEnabledV300` is expected to be `false`, `smartAdaptiveBypassedByForce30V302` is `true`, and `perlaForce30FpsV302(false)` must restore the normal adaptive path. V301 evidence includes `sampleClassesV301`, `steadyV301`, `transitionV301`, `warmupV301`, `protectedLargeSpriteStripeStepAppliedV301`, `floorMergedColorTransitionsV301`, `floorColorQuantCacheHitsV301`, and `coverageTransitionEmergencyAppliedV301`. V302 evidence includes `longViewPreviousFeedbackSuppressedV302`, `protectedLargeSpriteStableStripeStepAppliedV302`, `spriteStripeFeedbackSuppressedV302`, `floorStableQuantizedV302`, `floorStableBandAppliedV302`, `coverageModernCoverFarSkipSuppressedV302`, `coverageCanopyEmergencyStepSuppressedV302`, and `modernStableRoofPrimitiveAdaptiveStepSuppressedV302`.
