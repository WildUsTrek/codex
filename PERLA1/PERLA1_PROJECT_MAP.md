...
Current runtime build observed in `index.html`: `PERLA1_V302_STEADY_QUALITY_FORCE30_LOCAL`
...
| Runtime performance matrix | `tools/perla_runtime_perf_matrix.ps1`, `tests/perla_perf_critical_scenarios.json` | Tooling-only Playwright matrix runner for gameplay-first performance/draw evidence across clear/cloudy/rain/storm and desktop/mobile touch contexts. It starts or reuses the PERLA1 server safely, verifies `PERLA_BUILD_ID`, can load report-derived critical scenarios through `-ScenarioPath`, supports `-Weathers`, uses public debug APIs, writes JSON/Markdown/screenshots outside Git by default, and does not change runtime behavior. |
...
| V299 | active support (roof always budget) | Extends the V298 roof step-budget mechanism with a light non-storm full-surface roof fill budget for owner 1/2 modern roofs, so clear weather is covered while storm/rain continues to use the V298 pressure layer. V299 still applies only to `face.kind === 'roof'` full-surface fills, preserves edge lines, gable outlines, portal underside, fallback exclusivity, map/assets/rain output, and keeps V298 as the storm/rain escalation. Base build: `PERLA1_V299_ROOF_ALWAYS_BUDGET_SAFE_LOCAL`. |
| V300 | active base contract (adaptive performance safe) | Adds a measured smart-adaptive performance layer over V260 sprite/floor budgets, V261/V262 coverage budgets, V298 storm/rain roof pressure, and V299 non-storm roof fill budgets. V300 responds to clear/cloudy/rain/storm draw/frame/candidate/stripe/floor/coverage pressure without a hard 30 FPS cap, without changing worldgen, assets, maps, collision, rain output, or V281/V283/V292 visual authority. Base build: `PERLA1_V300_ADAPTIVE_PERFORMANCE_SAFE_LOCAL`. |
| V301 | active support (performance balance) | Adds residual runtime performance balance over V300: pre-adaptive boost for weather/zone/viewport/debug-pose transitions, staged environment cache invalidation, protected/local sprite-stripe guardrails, floor entropy quantization with per-frame color cache, transition-aware coverage recovery, and phase-separated smart reports (`steadyV301`, `transitionV301`, `warmupV301`). It preserves worldgen, assets, maps, collision, rain output, V281/V283/V292 authority, and avoids a hard fixed 30 FPS cap. Base build: `PERLA1_V301_PERFORMANCE_BALANCE_LOCAL`. |
| V302 | current contract (steady quality + force-30 switch) | Stabilizes steady-state visual quality by making floor/road quantization and far floor banding distance/row driven, making long-view sprite/floor budgeting scene-driven instead of previous-frame-driven in steady mode, making protected-large sprite stripe steps deterministic, suppressing far modern-cover skip and canopy emergency step alternation, and suppressing V300-driven roof primitive step changes. Adds `perlaForce30FpsV302(true/false)` as a real frame gate that defaults on, bypasses V300/V301 adaptive FPS while active, and is reversible through `localStorage`. It preserves worldgen, assets, maps, collision, rain output, and V281/V283/V292 authority. Current build: `PERLA1_V302_STEADY_QUALITY_FORCE30_LOCAL`. |

## V281 Modern Roof Primitive Authority Contract

Current expected behavior for modern reception/bath roof/eave work:

- Current `PERLA_BUILD_ID` is `PERLA1_V302_STEADY_QUALITY_FORCE30_LOCAL`; the V281 roof authority contract below remains active, with V300/V301 still providing measured adaptive pressure evidence while V302 prevents steady-state sprite/floor, floor/road, cover/canopy, and roof primitive quality from alternating on previous-frame pressure. V299/V298 keep their existing roof-budget ownership.
...
- V299 remains the always-covered roof draw budget: clear/non-storm owner 1/2 full-surface roof fills use the V299 light step budget, while storm/rain pressure uses the V298 counters. V300 may only increase measured adaptive quality budgets under pressure and must not step edge lines, gable outlines, portal underside, non-roof faces, unowned fallback paths, or rain output.
...
- `modernStableRoofPrimitiveV299 === true`; `modernStableRoofPrimitiveGeneralBudgetAppliedV299` and `modernStableRoofPrimitiveGeneralBudgetColumnsSavedEstimateV299` remain expected in owner 1/2 full-surface roof views in clear/non-storm weather
...
- `smartAdaptiveV300 === true`; with `force30FpsV302` off, `smartAdaptiveLevelV300`, `smartAdaptiveTargetFpsV300`, `longViewAdaptiveLevelV300`, and `coverageAdaptiveLevelV300` remain the V300 pressure evidence. With `force30FpsV302` on, `smartAdaptiveEnabledV300 === false` and `smartAdaptiveBypassedByForce30V302 === true` are expected. V301 proof adds phase-separated `sampleClassesV301` / `steadyV301` / `transitionV301` / `warmupV301`, transition preboost/staged invalidation counters, sprite stripe counters, floor entropy/cache counters, and coverage transition emergency counters. V302 proof adds `longViewPreviousFeedbackSuppressedV302`, `protectedLargeSpriteStableStripeStepAppliedV302`, `spriteStripeFeedbackSuppressedV302`, `floorStableQuantizedV302`, `floorStableBandAppliedV302`, `coverageModernCoverFarSkipSuppressedV302`, `coverageCanopyEmergencyStepSuppressedV302`, and `modernStableRoofPrimitiveAdaptiveStepSuppressedV302`.
...
- `tools/perla_runtime_perf_matrix.ps1` for measured clear/cloudy/rain/storm desktop/mobile performance matrix evidence. Use `tests/perla_perf_critical_scenarios.json` with `-ScenarioPath` for report-derived hotspot validation. Default output is `%TEMP%\PERLA1_perf_matrix_<timestamp>`; generated screenshots/reports remain disposable unless explicitly promoted.
...
| `rtp-scenario-workflow-planner` | `workspace-write` | Maintains `RTP_SCENARIO_WORKFLOW_ROADMAP.md`: current RTP/scenario phase, active milestone, task start/end state, blockers, validation evidence, completed delta, and next step. |
...
- For RTP/scenario work, `rtp-scenario-workflow-planner` is `CALL` at start and end for non-trivial tasks so `RTP_SCENARIO_WORKFLOW_ROADMAP.md` records the current phase, active milestone, completed delta, validation evidence, blockers, and next step. `scenario-rtp-map-auditor`, `map-placement-auditor`, `event-flow-auditor`, and `dialogue-continuity-auditor` are mandatory domain auditors when their signals match. They do not replace asset integrity, code mapping, renderer, visual QA, workflow guard, workflow consistency, plan integrity, watchdog, or runtime validation.
